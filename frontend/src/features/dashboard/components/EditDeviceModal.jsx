...
const EditDeviceModal = ({ isOpen, onClose, onDeviceUpdated, device }) => {
    ...
    useEffect(() => {
        if (device) {
            ...
            setFormData({
                name: device.name || '',
                model: device.model || '',
                type: device.type || 'Android TV',
                osVersion: specs.osVersion || '',
                ipAddress: specs.ipAddress || '',
                backendUrl: backendUrl,
                status: device.status || 'offline'
            });
            setBackendSystem(matchingBackend);
        }
    }, [device]);
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl scale-100 animate-in zoom-in-95 duration-200 overflow-visible">
                ...
                <div className="flex items-start justify-between p-6 border-b border-gray-100">
                    <div className="flex gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 leading-tight">Edit Device</h2>
                            <p className="text-sm text-gray-500 mt-1">Update device configuration.</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
