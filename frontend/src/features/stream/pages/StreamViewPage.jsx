...
const StreamViewPage = () => {
    ...
    useEffect(() => {
                ...
                if (disconnectedDeviceId === deviceId) {
                    ...
                    const clearViewer = async () => {
                        try {
                            const token = localStorage.getItem('token');
                            if (token) {
                                await fetch(`http://localhost:5000/api/devices/${deviceId}/clear-viewer`, {
                                    method: 'POST',
                                    headers: {
        ...
        const handleBeforeUnload = async () => {
            ...
            if (token) {
                ...
                fetch(`http://localhost:5000/api/devices/${deviceId}/clear-viewer`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
    ...
    const checkSessionStatus = async (sessionId) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/sessions/${sessionId}/status`);
            if (response.ok) {
                const data = await response.json();
