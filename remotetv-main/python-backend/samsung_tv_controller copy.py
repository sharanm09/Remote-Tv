...
class SamsungTVController:
        ...
        def _connect():
            try:
                ...
                try:
                    ...
                    # Even if it's the same as stored_token, we want to make sure it's saved
                    ...
                    elif stored_token and tv_name:
                        # If we have a stored token but couldn't extract a new one, ensure it's saved
    ...
    async def ensure_connection(session):
        ...
        async with session.samsung_connection_lock:
            try:
                now = time.time()
                # Always check if existing connection is actually alive (don't trust cached state)
                if session.samsung_tv:
                    # Test connection by trying to check socket state
                    def _check_alive():
                        return SamsungTVController._check_connection_alive(session.samsung_tv)
                    is_alive = await asyncio.to_thread(_check_alive)
                    if is_alive:
                        # Connection appears alive, but for lazy connections we can't be 100% sure
                        # Only skip reconnection if we checked very recently (within 2 seconds)
                        if now - session.samsung_last_ensure_ts < 2:
                            return session.samsung_tv
                        # Otherwise, continue to verify connection is still good
                    else:
                ...
                # If we still have a connection after the checks above, it should be good
                # But if we cleared it (because it was dead), we need to create a new one
                if session.samsung_tv:
                    # Connection passed the alive check above, update timestamp and return it
                    session.samsung_last_ensure_ts = now
                    logger.debug(f"Reusing existing connection for session {session.id}")
                    return session.samsung_tv

                # Only create new connection if we don't have one or it's dead
                # IMPORTANT: Always use session token first (most recent), then fall back to stored token
                stored_token = None
                if hasattr(session, 'samsung_token') and session.samsung_token:
                    # Use token from session first (most recent)
                    stored_token = session.samsung_token
                    logger.info(f"Using token from session for {session.ip}")
                elif session.tv_name:
                    # Fall back to token manager if session doesn't have token
                    stored_token = token_manager.get_samsung_token(session.tv_name)
                    if stored_token:
                        logger.info(f"Using stored token from token_manager for {session.tv_name}")
                        session.samsung_token = stored_token  # Update session with token
                
                # IMPORTANT: Always use stored token if available to avoid re-authentication
                if not stored_token and session.tv_name:
                    logger.warning(f"No stored token found for {session.tv_name}, will require TV authentication")
                ...
                def _open_ws():
                    ...
                    # ALWAYS try with stored token first if available - this prevents re-authentication
                    if stored_token:
                        try:
                            logger.info(f"Attempting to connect with stored token for {session.ip} (token length: {len(stored_token)})")
                            # Create connection with token - this should prevent authentication popup
                            tv = _WS(session.ip, port=8002, token=stored_token, name="SamsungTvRemote")
                            # SamsungTVWS uses lazy connections - object creation is enough
                            # Connection will be established when first command is sent
                            # The token will be used automatically when connection is established
                            logger.info(f"WebSocket connection object created with stored token for {session.ip}")
                            return tv
                        except Exception as e:
                            logger.warning(f"Stored token failed for {session.ip}: {e} - token may be invalid or expired")
                            last_error = e
                            tv = None
                            # Don't immediately fall back - log the error but try without token as last resort
                    
                    # Only try without token if we have no token or token failed
                    # This will trigger authentication popup, but it's better than failing completely
                    if not tv:
                        if stored_token:
                            logger.warning(f"Token connection failed, but will try without token as fallback (will require TV auth)")
                        else:
                            logger.info(f"No token available, connecting without token (will require TV auth)")
                        try:
                            ...
                            error_msg = f"Failed to create WebSocket connection object: {e}"
                            if last_error:
                                error_msg += f" (previous error with token: {last_error})"
                            logger.error(error_msg)
                ...
                # Extract and save token - always ensure session has the token
                # For lazy connections, token might not be available until first use
                # But we should always store the token we're using
                if stored_token:
                    # Always store the token we're using in session
                    session.samsung_token = stored_token
                    logger.info(f"Stored token in session for {session.ip}")
                
                # Try to extract new token from connection (in case we got a new one)
                new_token = None
                try:
                    # Try multiple ways to get token from SamsungTVWS
                    if hasattr(tv, 'token') and tv.token:
                        new_token = tv.token
                    elif hasattr(tv, 'connection') and hasattr(tv.connection, 'token'):
                        new_token = tv.connection.token
                    # Also check if token_file was used
                    ...
                    logger.debug(f"Could not extract token from connection object: {e}")
                
                # Save token if we got a new one (different from stored)
                ...
                elif stored_token and session.tv_name:
                    # Ensure token is saved in token_manager if not already there
                    existing_token = token_manager.get_samsung_token(session.tv_name)
                    if not existing_token or existing_token != stored_token:
                        token_manager.set_samsung_token(session.tv_name, stored_token)
                        logger.info(f"Saved token to token_manager for {session.tv_name}")
    ...
    async def send_key(tv, key: str):
        ...
        def _send_key():
            try:
                ...
                if any(keyword in error_msg or keyword in error_type for keyword in connection_errors):
                ...
                return {"status": "error", "error": error_msg, "connection_dead": False}
    ...
    async def send_command(session, msg):
        ...
        if msg.get("type") == "key":
            ...
            if result.get("status") == "error" and result.get("connection_dead", False):
            ...
            if (result.get("status") == "success" or ("Connection" not in result.get("error", ""))) and hasattr(session, 'samsung_logs_enabled') and session.samsung_logs_enabled:
                ...
                asyncio.create_task(SamsungTVController._capture_logs_background(session, action, delay=0.5))
            ...
            if result.get("status") == "success" and hasattr(session, 'samsung_logs_enabled') and session.samsung_logs_enabled:
                asyncio.create_task(SamsungTVController._capture_logs_background(session, f"app_{app_id}", delay=1.0))
            return result
            ...
            try:
                ...
                # Only capture logs if user has enabled log capture
                if hasattr(session, 'samsung_logs_enabled') and session.samsung_logs_enabled:
                    asyncio.create_task(SamsungTVController._capture_logs_background(session, "text_input", delay=0.5))
