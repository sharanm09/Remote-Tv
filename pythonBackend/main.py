...
async def verify_tv_connection(session: Session) -> bool:
    ...
    try:
        if session.device_type == DeviceType.SAMSUNG_TV:
            # Check Samsung TV REST API (WebSocket connection is lazy and handled by ensure_connection)
            def _check_samsung():
                try:
                    from samsungtvws import SamsungTVWS
                    # Check REST API (port 8001) - this verifies TV is on
                    tv_rest = SamsungTVWS(session.ip, port=8001, name="SamsungTvRemote")
                    tv_rest.rest_device_info()
                    logger.debug(f"Samsung TV REST API check successful for {session.ip}")
                    # REST API works, so TV is on
                    # WebSocket connection is lazy - it will be established by ensure_connection when needed
                    # If session has connection object, it's ready to use
                    if hasattr(session, 'samsung_tv') and session.samsung_tv:
                        logger.debug(f"Samsung TV WebSocket connection object exists for {session.ip}")
                    return True
                except Exception as e:
...
async def send_command_to_device(session_id: str, payload: Payload):
    ...
    # Verify TV is actually connected before sending command
    # For Samsung TV, we'll verify connection in the controller itself (it handles lazy connections)
    # For other TVs, verify connection here
    if session.status == SessionStatus.CONNECTED:
            if session.device_type != DeviceType.SAMSUNG_TV:
                # For non-Samsung TVs, verify connection first
                is_connected = await verify_tv_connection(session)
                if not is_connected:
                    # TV is off or unreachable, update session status
                    session.status = SessionStatus.DISCONNECTING
                    session.update_activity()
                    logger.warning(f"TV at {session.ip} is off or unreachable, cannot send command to session {session_id}")
                    return {
                        "status": "error",
                        "error": "TV is off or unreachable. Please turn on the TV and reconnect.",
                        "connected": False
                    }
            else:
                # For Samsung TV, verify REST API is reachable (WebSocket connection is handled in controller)
                # This is faster than full connection check
                try:
                    def _quick_check():
                        from samsungtvws import SamsungTVWS
                        tv_rest = SamsungTVWS(session.ip, port=8001, name="SamsungTvRemote")
                    ...
                    is_reachable = await asyncio.wait_for(asyncio.to_thread(_quick_check), timeout=3.0)
                    if not is_reachable:
                        logger.warning(f"Samsung TV at {session.ip} is not reachable")
                        session.status = SessionStatus.DISCONNECTING
                        session.update_activity()
                        return {
                            "status": "error",
                    ...
                    logger.warning(f"Samsung TV reachability check failed for {session.ip}: {e}")
                    session.status = SessionStatus.DISCONNECTING
                    session.update_activity()
                    return {
                        "status": "error",
